...
public class MathUtil{
	...
	public static final Random weakRand = Rand.weakRand;
	public static final SecureRandom strongRand = Rand.strongRand;
	...
	*/
	...
	public static double max(double d[]){
		...
		for(double dd : d) if(max < dd) max = dd;
		return max;
	}
	
	public static void scaleLinearlyToFractionRange(double d[]){
		double min = min(d), max = max(d);
		if(min == max) Arrays.fill(d, .5);
		else for(int i=0; i<d.length; i++){
			d[i] = (d[i]-min)/(max-min);
		}
	}
